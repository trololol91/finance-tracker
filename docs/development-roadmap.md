...
- ✅ `push/` module — Web Push + email for MFA alerts (implementation done; API tests complete: 18/18 pass; OS toast E2E blocked on Desktop MCP server)
...
3. ✅ `@backend-dev` — **Carry-over C**: Admin endpoints implemented — `POST /admin/scrapers/reload` and `POST /admin/scrapers/install` (ADMIN role only). `AdminGuard` added to `common/guards/`, exported from `CommonModule`, and registered in `ScraperModule.providers`. `ScraperAdminService` and `ScraperAdminController` added to `ScraperModule`. 23 unit tests. Commits: `b2d9200`, `9df08e2`, `bc46927`, `4c6b7df`, `8900746`.
4. ✅ `@backend-dev` — **Carry-over D**: `push/` module implemented — `POST /push/subscribe`, `DELETE /push/subscribe`, Web Push VAPID `sendNotification`, nodemailer email fallback. Wired into `ScraperService.handleMfaRequired()`. 23 unit tests. Commits: `a2f7e35` (implementation), `16957c2` (review fixes: redundant module, HTML escape, SMTP_PORT parseInt, requireTLS, 204 response, export DTO, @ApiResponse), `18104f2` (NaN guard, type safety), `f063d88` (guard ordering fix — NaN check moved after missing-vars graceful-disable path).
5. ✅ `@backend-tester` — **Carry-over D API tests**: 21-TC live API test plan + report (`test-plan/push/backend.md`, `backend-report.md`). 18/18 exercisable TCs passed, 0 bugs. 3 TCs skipped (VAPID delivery, SMTP email, OS toast) — all require external services unavailable in dev. Auth guards, all validation paths, idempotency, 204 No Content, and clean error response shapes all verified. Commit: `test(push): add backend API test plan and report`.

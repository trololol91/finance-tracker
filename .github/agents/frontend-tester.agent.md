...
- **Cross-feature tests**: When a feature change adds UI or behaviour to an existing feature's route (e.g. wiring a `categoryId` selector into the Transactions page), add the new TCs to the **existing** `test-plan/<feature>/frontend.md` and results to `test-plan/<feature>/frontend-report.md` — do **not** create a separate file. A separate file is only justified for a genuinely new, standalone feature that has no existing plan document.
  - **Place TCs in their natural section**, not in a separate appendix. A TC that tests the Category filter belongs under **Filters**; one that tests the Add modal belongs under **Add Transaction**; a responsive column-visibility TC belongs under **Responsive Layout**. Only create a new section heading when the cross-feature change introduces a genuinely new interaction area that has no existing section (e.g. a brand-new "Bulk Actions" toolbar added to an existing page).

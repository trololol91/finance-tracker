...
export class GoogleDriveDirectoryService {
     ...
     */
    async createDirectory(name: string, parentId?: string): Promise<drive_v3.Schema$File> {
        try {
    ...
    async moveDirectory(
        ...
        try {
            ...
            this.logger.error(
                `Failed to move directory ID: ${directoryId} to parent: ${newParentId}`,
                error,
            );
            throw error;
        }
     ...
     * @param recursive Whether to recursively delete contents (if false and directory is not empty, will throw an error)
